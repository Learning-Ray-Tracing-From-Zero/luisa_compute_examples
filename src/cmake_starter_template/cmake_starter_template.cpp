...
int main(int argc, char *argv[]) {
    ...
    // Step 1.2: Load backend plug-in and create a device
    Device device = context.create_device(argv[1]);

    // Step 2.1: Create a stream for command submission
    Stream stream = device.create_stream();

    // Step 2.2: Create an 1024x1024 image with 4-channel 8-bit storage for each pixel;
    //   the template argument `float` indicates that
    //   pixel values reading from or writing to the image are converted from `byte4` to `float4` or
    ...
    //   A `Callable` is a function *entity* (not directly inlined during the AST recording) that
    //   is invocable from kernels or other callables
    // Float4: alias for Var<float4>
    ...
    // ImageFloat: alias for Var<Image<float>>
    Kernel2D fill_image_kernel = [&linear_to_srgb](ImageFloat image) noexcept {
        ...
        image->write(
            coord,
            linear_to_srgb(make_float4(rg, 1.0f, 1.0f))
        );
    };

    // Step 3.2: Compile the kernel into a shader (i.e., a runnable object on the device)
    Shader fill_image = device.compile(fill_image_kernel);
    std::vector<std::byte> download_image(1024u * 1024u * 4u); // Prepare the host memory for holding the image

    // Step 4: Generate commands from resources and shaders,
    //   and submit them to the stream to execute on the device
