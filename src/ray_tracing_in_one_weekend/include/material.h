...
public:
    ...
    // virtual bool is_equal(const luisa::shared_ptr<material>& other) const = 0;
};


class lambertian : public material {
public:
    luisa::shared_ptr<texture> albedo;

public:
    lambertian(const float3 &a)
    ...
    // virtual bool is_equal(const luisa::shared_ptr<material>& other) const override {
    //     if (typeid(*this) != typeid(*other)) {
    //         return false;
    //     }
    //     auto derived = luisa::static_pointer_cast<lambertian>(other);
    //     return this->albedo == derived->albedo;
    // }
};
