...
int main(int argc, char *argv[]) {
    ...
    // Camera
    float3 lookfrom { 0.0f, 0.0f, 0.0f };
    ...
    switch (options["scene"].as<int>()) {
        ...
        case 5: {
            ...
            background = float3 { 0.0f, 0.0f, 0.0f };
            lookfrom = float3 { 26.0f, 3.0f, 6.0f };
            lookat = float3 { 0.0f, 2.0f, 0.0f };
            break;
        }
        case 6: {
            world = cornell_box();
            ...
            image_width = 600;
            background = float3 { 0.0f, 0.0f, 0.0f };
            lookfrom = float3 { 278.0f, 278.0f, -800.0f };
            lookat = float3 { 278.0f, 278.0f, 0.0f };
            vfov = 40.0f;
            break;
        }
        case 7: {
    ...
    // Render
    uint image_height = static_cast<uint>(
        static_cast<float>(image_width) / aspect_ratio
    );
    uint2 resolution = make_uint2(image_width, image_height);
    auto seed_image = device.create_image<uint>(PixelStorage::INT1, resolution, 1u, false, false);
    auto accum_image = device.create_image<float>(PixelStorage::FLOAT4, resolution, 1u, false, false);
    ...
    Kernel2D gamma_kernel = [&](ImageFloat accum_image, ImageFloat output) {
        ...
        output.write(
            coord,
            make_float4(
                sqrt(accum_image.read(coord).xyz()),
                1.0f
            )
        );
    };
...
cxxopts::ParseResult parse_cli_options(
    ...
    cli.set_width(80);
    cli.add_option("", "", "help", "Display this help message", cxxopts::value<bool>()->default_value("false"), "");
    cli.add_option("", "", "backend", "Compute backend name", cxxopts::value<luisa::string>()->default_value("cpu"), "<backend>");
    cli.add_option("", "", "samples", "Samples per pixel", cxxopts::value<std::size_t>()->default_value("100"), "<numbers>");
    cli.add_option("", "", "outfile", "Output image file name", cxxopts::value<luisa::string>()->default_value("./test"), "<image_name>");
    cli.add_option(
        "",
        "",
        "scene",
