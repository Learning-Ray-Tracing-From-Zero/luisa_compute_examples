...
__global__ void kernel(float* in_arr, float* out_arr, int n) {
    ...
    shared_arr[index_pout] = (thread_x > 0 && thread_x < n) ? in_arr[thread_id - 1] : 0;
    __syncthreads();

    for (int offset = 1; offset < n; offset *= 2) {
        previous_out = 1 - previous_out; // swap double buffer indices
        previous_in = 1 - previous_out;
        index_pout = previous_out * n + thread_x;
        index_pin = previous_in * n + thread_x;
    ...
    // Write output
    if (thread_x < n) { out_arr[thread_id] = shared_arr[index_pout]; }
}
